...
io.on('connection', (socket) => {
  ...
  socket.on('join', (data: { username: string }, ack?: (result: {
    ok: boolean
    user?: User
    users?: User[]
    error?: string
    ...
    // Confirm join only to this client (fixes race with UI)
    socket.emit('join-success', { user, users: usersList })
    ack?.({ ok: true, user, users: usersList })
